...
import { type AliadoPerfil, ETIQUETA_TIPO, type TipoAliado } from './epica02'
...
export function AliadoDetallePage() {
  ...
  const editar = useMutation({
    mutationFn: (datos: Record<string, unknown>) => apiFetch(`/aliados/${id}`, { method: 'PATCH', body: JSON.stringify(datos) }),
  ...
  const error = estado.error ?? editar.error
    ...
    <>
      ...
      <section className="card">
        ...
        {editando ? (
          ...
          <dl><dt>Identificación</dt><dd>{datos.identificacion}</dd><dt>Tipo</dt><dd>{ETIQUETA_TIPO[datos.tipo as TipoAliado]}</dd><dt>Ciudad</dt><dd>{datos.ciudad ?? '—'}</dd><dt>Correo</dt><dd>{datos.correo ?? '—'}</dd><dt>Teléfono</dt><dd>{datos.telefono ?? '—'}</dd></dl>
        )}
