...
class HistorialEtapa(Base):
    ...
    etapa_origen_id: Mapped[int | None] = mapped_column(ForeignKey("etapa.id"), nullable=True)
    etapa_destino_id: Mapped[int] = mapped_column(ForeignKey("etapa.id"), nullable=False)
    ...
    numero_ciclo: Mapped[int] = mapped_column(SmallInteger, nullable=False)
    ...
    etapa_origen: Mapped[Etapa | None] = relationship(foreign_keys=[etapa_origen_id])
    ...
    observaciones: Mapped[list[ObservacionRevision]] = relationship(back_populates="historial_etapa")
