...
_ESTADOS = ", ".join(f"'{valor.value}'" for valor in EstadoObservacionRevision)
...
class ObservacionRevision(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"origen IN ({_ORIGENES})", name="ck_observacion_revision_origen"
        ),
        CheckConstraint(
    ...
    estado: Mapped[str] = mapped_column(
        String(20),
        default=EstadoObservacionRevision.PENDIENTE.value,
        server_default=EstadoObservacionRevision.PENDIENTE.value,
        nullable=False,
        index=True,
    )
    ...
    atendida_por: Mapped[Usuario | None] = relationship(
