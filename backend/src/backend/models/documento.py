...
class Documento(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "tipo IN ('CAMARA_COMERCIO', 'RUT', "
            "'CEDULA_REPRESENTANTE_LEGAL', 'OTRO_DOCUMENTO_REPRESENTACION', "
            "'OTRO_SOPORTE', 'BORRADOR', 'AVAL_JURIDICO', 'CONVENIO_FIRMADO', "
            "'OTROSI', 'ACTA_TERMINACION')",
    ...
    solicitud_id: Mapped[int | None] = mapped_column(
        ForeignKey("solicitud_convenio.id", ondelete="CASCADE"), nullable=True, index=True
    )
    ...
    ruta_almacenamiento: Mapped[str] = mapped_column(String(255), unique=True, nullable=False)
    version: Mapped[int | None] = mapped_column(SmallInteger, nullable=True)
    es_vigente: Mapped[bool] = mapped_column(Boolean, default=True, server_default=true(), nullable=False)
    ...
    solicitud: Mapped[SolicitudConvenio | None] = relationship(back_populates="documentos")
